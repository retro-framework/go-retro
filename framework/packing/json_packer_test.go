...
func Test_UnpackPack(t *testing.T) {
	...
	t.Run("exemplary affix", func(t *testing.T) {
		...
		var (
			jp    = NewJSONPacker()
			hash  = hashStr("foo")
			affix = Affix{"baz/123": []retro.Hash{hash}, "bar/123": []retro.Hash{hash}}
		)
	...
	t.Run("exemplary checkpoint", func(t *testing.T) {
		...
		var (
			...
			checkpoint     = Checkpoint{
				...
				ParentHashes: []retro.Hash{checkpointHash},
			}
		)
...
func Test_Pack(t *testing.T) {
	...
	t.Run("exemplary affix", func(t *testing.T) {
		...
		// Act
		res, err := jp.PackAffix(Affix{"baz/123": []retro.Hash{hash}, "bar/123": []retro.Hash{hash}})
